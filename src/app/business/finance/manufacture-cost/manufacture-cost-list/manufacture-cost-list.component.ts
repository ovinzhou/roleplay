...
@Component({
  selector   : 'manufacture-cost-list',
  templateUrl: './manufacture-cost-list.component.html',
  styleUrls  : ['./manufacture-cost-list.component.scss']
})

export class ManufactureCostListComponent implements OnInit {

  private filter          : any;
  private totalRecords    : any;
  private page            : any;  
  private manufactureCostList      : any;
  private data            : any;
  ...
  ngOnInit(){
    ...
    this.toolbarConfigs = [
      ...
      {
        label: "报销",
        handler: this.onTsovhdrb.bind(this),
        align:'right',
        icon:'glyphicon glyphicon-plus'
      }
    ]
  ...
  public setManufactureCostData(){
    this.service.getManufactureCostList(this.filter).then(data=>{
  ...
  public formatData(data,type){
    if(type == ''){
      ...
      this.manufactureCostList = data.content;
      this.manufactureCostList.map(item => {
  ...
  public setManufactureDetailList(){
